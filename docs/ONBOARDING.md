...
세 축을 겹쳐서 나눴습니다. 한 사람이 한 레포를 통째로 맡지 않습니다.

**① AI 는 에이전트 담당 그대로.** 개발보고서의 7개 에이전트를 네 명이 나눠 맡기로 한 원래 합의가 있습니다. 그 배정을 손대지 않았습니다. 알고리즘 명세서도 같은 기준으로 배정돼 있어 문서와 코드가 한 사람에게 모입니다.

**② 스프링은 자기가 맡은 화면의 라우트.** 에이전트로 나누면 스프링에 통관 라우트 같은 게 없어 배분이 되지 않습니다. 화면 담당은 프론트 UI 를 만들 때 이미 정해져 있었으니 그대로 이어받습니다. 화면을 만든 사람이 그 화면에 필요한 데이터를 제일 잘 압니다.

| 사람 | 맡은 화면 | 그래서 가져가는 스프링 라우트 |
|---|---|---|
| 차은호 | 로그인 · 회원가입 · 분석보고서 | 레포 골격, auth, products·uploads, analysis 프록시와 보고서 매핑 |
| 권수현 | 대시보드 · 판매정보 | dashboard, sales-info·quote·categories, sales-ops |
| 이동건 | 마켓설정 · 판매관리 | settings·stores, 코파일럿 프록시 |
| 강근우 | 상품등록 · 보고서 · 상세페이지 | detail-page, images, image-jobs |

**③ 프론트 API 연결은 이동건과 강근우.** 두 사람이 이걸 맡는 만큼 AI 와 스프링 분량을 줄였습니다. 스프링만 봐도 이동건 4, 강근우 4인데 차은호 13, 권수현 11입니다.

산출물 문서는 별도 배정이라 개발 담당과 다를 수 있습니다. 오케스트레이션 명세서는 개발이 강근우인데 문서는 권수현입니다. 이것도 팀이 정한 그대로 뒀습니다.
